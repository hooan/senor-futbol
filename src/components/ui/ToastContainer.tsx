...
function ToastItem({ toast, onRemove }: ToastItemProps) {
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (element) {
        element.classList.add('animate-toast-in')
      }
    }, 10)
  ...
  return (
    <div
      id={toast.id}
      className={`flex items-start gap-3 p-4 border-thick shadow-lg opacity-0 transition-all duration-300 pointer-events-auto ${getStyles()}`}
